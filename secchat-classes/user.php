...
{
	die('No connection to DB or Session is not started!');
}
...
<li>"Запрещённый IP адрес" - пользователь ввёл правильную пару имени и пароля, но ему запрещено работать с этого IP адреса. Данное событие может быть расценено как неправомерный доступ к системе управления сайтом, потребуйте объяснений от пользователя. Скорее всего он пытался получить доступ к системе с сотового телефона, из дома, из интернет кафе и прочих мест, где не может быть обеспечен требуемый уровень безопасности для работы с системой. Если пользователь не может дать внятных объяснений, кто и почему пытался зайти в систему управления сайтом, рекомендуется сменить пароль пользователя.</li>
