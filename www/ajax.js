...
$('textarea[name=new_mesg]').keypress(function(e)
    {
        if(e.which == 13)
        {
        $.post(basedir+'/handlers/msg_handler', {'s': $('input[name=s]').val(),'URI':$('#URI').html(), 'new_mesg': encodeURIComponent($('textarea[name=new_mesg]').val())}, function(data)
        {
...
{
$.post(basedir+'/handlers/validator',{'s': $('input[name=s]').val(), 'pwd': $('input[name=new_pwd_val]').val()},function(data)
    {
...
{
    $.post(basedir+'/handlers/validator',{'s': $('input[name=s]').val(), 'select_U': $('input[name=adduser2channel]').val()},function(data)
    {
...
{
    $.post(basedir+'/handlers/validator',{'s': $('input[name=s]').val(), 'new_U': $('input[name=create_new_user]').val()},function(data)
    {
...
var num_comments;
var c;
$.post(basedir+'/handlers/msg_handler', {'s': s,'URI': URI,'count': 1}, function(data)
        {
        if(data>$('#num_mesg').html()+1)
            ...
            $('#num_mesg').html(data);
            $.post(basedir+'/handlers/msg_handler', {'s': s,'URI': URI,'list': c}, function(data)
            {
            $('#num_mesg').remove();
            $('#channel_mesg_placeholder').append(data);
            $('#num_mesg').html(num_comments);
            $('#new_comment').fadeOut('slow')  .fadeIn('slow');
            $('#new_comment').attr("id",'');
            scrolling=true;
            });
            }
